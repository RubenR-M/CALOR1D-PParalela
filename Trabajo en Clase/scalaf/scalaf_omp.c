...
Compilacion
gcc -o scalaf_omp -fopenmp scalaf_omp.c -lm -w
g++ -o scalaf_omp -fopenmp scalaf_omp.c -lm -w
...
Ejecucion
./scalaf_omp -t 1500 -v 100 -w 1 -s crater_location -a altitudesMil.csv -r 1000 -c 1000 -p 10 -e "1000x1000" -n 10
./scalaf_omp -t 1500 -v 100 -w 1 -s crater_location -a altitudesMil.csv -r 1024 -c 1024 -p 1 -e "1024x1024" -n 10
                 ...
                 int totalCraters)
{
  ...
  int craterColumn, craterRow, i;

//  double start2 = omp_get_wtime();
#pragma omp parallel for private(i)
  for (i = 0; i < totalCraters; ++i)
  {
    craterRow = P[i].x;
    craterColumn = P[i].y;
...
//  double stop2 = omp_get_wtime();
  ...
  // printf("\tTiempo 'placeCraters' (omp): %f seconds. \n", (stop2 - start2) );
}
...
void preFuncion(int filas, int columnas, const mapCell *A, mapCell *C)
{
  ...
  f = 0;
//  double start2 = omp_get_wtime();
  //  #pragma omp parallel for // private(f) //reduction(+:f) // 178 / GRAFICA -> RED(vacio), SOLO(varios) / NO ES POSIBLE USAR EL private(f)
  for (i = 0; i < filas + 2; ++i)
  {
    if (!(i == 0 || i == (filas + 1)))
    {
...
//  double stop2 = omp_get_wtime();
  // printf("\tTiempo 'preFuncion' (omp): %f seconds. \n", (stop2 - start2) );
...
#pragma omp parallel for reduction(+: f) private(i) schedule(dynamic) // se mantiene aprox igual con el schedule
  for (i = 0; i < filas; i++)
  {
    ...
    {
      c = 0;
#pragma omp simd reduction(+: c) private(j) // simd (no schedule) / parallel for -> aprox igual
...
//  double stop2 = omp_get_wtime();
  // printf("\tTiempo 'postFuncion' (omp): %f seconds. \n", (stop2 - start2) );
                                   ...
                                   double y0)
{
  ...
  if (sizep != 0)
  {
    ...
    if (datosAltitud != NULL)
    {
      ...
      cont = 0;
//      double start2 = omp_get_wtime();
      //      #pragma omp parallel for private(j) reduction(+:cont)//solo,private,red->G,T /
      for (j = 1; j < columnas - 1; ++j)
      { // aca debo cambiar esto, altitude +
        // thickness da la altura, osea la z
        //        #pragma omp parallel for private(i) reduction(+:cont) //solo,red,priv->G,T
        for (i = 1; i < filas - 1; ++i)
        {
          xcoord = x0 + (i - 1) * w;
          ycoord = y0 + (j - 1) * w;
...
//      double stop2 = omp_get_wtime();
      // printf("\tTiempo 'prepararVisualizacionGNUPlot_2' (omp): %f seconds. \n", (stop2 - start2) );
...
//  double stop2 = omp_get_wtime();
  ...
  // printf("\tTiempo 'limpiarPath' (omp): %f seconds. \n", (stop2 - start2) );
  f_path[j] = '\0';
  strcpy(spath, f_path);
}
